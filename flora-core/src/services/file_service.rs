...
impl FileService {
    ...
    fn validate_filename(name: &str) -> Result<()> {
        if name.contains('/') || name.contains('\\') || name.contains("..") {
            return Err(Error::InvalidInput(
                "invalid filename: path traversal characters are not allowed".to_string(),
            ));
        }
        Ok(())
    }
    ...
    pub async fn create_file(
        ...
        // Enforce organization quota (10GB default per FR-032)
        if size_bytes > self.config.org_quota_bytes {
            return Err(Error::FileTooLarge {
                size: size_bytes,
                max: self.config.org_quota_bytes,
            });
        }

        // Enforce workspace quota (2GB default per FR-032)
    ...
    pub async fn list_files(
        ...
        self.file_repo
            .find_by_workspace_id(workspace_id, pagination)
            .await
    }
...
mod tests {
    ...
    fn test_validate_filename() {
        assert!(FileService::validate_filename("valid_file.txt").is_ok());
        assert!(FileService::validate_filename("path/traversal.txt").is_err());
        assert!(FileService::validate_filename("../parent.txt").is_err());
    }

    #[test]
    fn test_validate_mime_type() {
        assert!(FileService::validate_mime_type("image/png").is_ok());
        assert!(FileService::validate_mime_type("application/pdf").is_ok());
        assert!(FileService::validate_mime_type("application/x-executable").is_err());
        assert!(FileService::validate_mime_type("application/x-sh").is_err());
    }
}
